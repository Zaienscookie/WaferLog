    ...
    <div className="page">
      ...
      <div className="detail-grid">
        <section>
          ...
          {note.audioUrl && (
            <section className="note-audio" aria-label="Audio attachment">
              <div className="note-audio-head">
                <span className="note-audio-title">
                  <Icon name="mic" width={16} height={16} />
                  Audio attachment
                </span>
                <span className="muted">{formatAudioSize(note.audio?.size || 0)}</span>
              </div>
              <audio controls preload="metadata" src={note.audioUrl} />
              {note.audio && (
                <div className="note-audio-meta muted">
                  {note.audio.sampleRate || 16000} Hz · {note.audio.channels || 1} ch · {note.audio.bits || 16} bit
                </div>
              )}
            </section>
          )}
